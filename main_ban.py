...
# Training settings
parser = argparse.ArgumentParser(description='Bag Attention Network code')
...
parser.add_argument('--config', default='utils/settings.yaml',type=str)
...
ckpt_dir_ban = osp.join(cfg.General.ckpt_dir, args.dataset, args.description, 'ban')
ckpt_file_ban = osp.join(ckpt_dir_ban, 'accs.tar')
...
if __name__ == "__main__":   
    ...
    accs, curr_run, curr_fold = load_checkpoint(args.run, args.folds, ckpt_file_ban)
    ...
    for irun in range(curr_run, args.run):
        ...
        for ifold in range(curr_fold, args.folds):
            ...
            print("\nTRAIN BAN MODEL:")
            ban_model = Model_with_embs(args2)
            for e in range(args2.epochs):
                ban_model.train()
            best_model_ban = ban_model.get_best_model()
            accs[irun][ifold] = 1 - best_model_ban.eval_error()[0]
            save_checkpoint(accs, irun, ifold, args.folds, ckpt_file_ban)
        curr_fold = 0
    ...
    os.remove(ckpt_file)   
    os.remove(ckpt_file_ban) 
    save_results(accs_base, results_dir, args.model)
    save_results(accs, results_dir, 'ban')
